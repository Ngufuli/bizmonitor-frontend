...
const CashPage = ({bizId, userRole}) => {
  ...
  const [form,setForm]=useState({date:todayStr(),opening_balance:"",closing_balance:"",notes:""});
  ...
  const submit=async()=>{
    if(!form.opening_balance||!form.closing_balance) return showToast("Enter both balances",C.red);
    setSubmitting(true);
    try{
      await apiPost(`/businesses/${bizId}/cash`,{...form,opening_balance:Number(form.opening_balance),closing_balance:Number(form.closing_balance)});
  ...
  const chartData=balances.slice(0,30).reverse().map(b=>({date:String(b.date).slice(5),opening:b.opening_balance,closing:b.closing_balance}));
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:22}}>
        <KpiCard label="Latest Closing" value={latest?fmtFull(latest.closing_balance):"—"} sub={latest?String(latest.date):"No records"} color={C.green} trend={movement>=0?"up":"down"}/>
        <KpiCard label="Latest Opening" value={latest?fmtFull(latest.opening_balance):"—"} sub="Most recent" color={C.blue}/>
        <KpiCard label="Daily Movement" value={latest?fmtFull(Math.abs(movement)):"—"} sub={movement>=0?"Positive":"Negative"} color={movement>=0?C.green:C.red} trend={movement>=0?"up":"down"}/>
        <KpiCard label="Avg Closing"    value={fmtFull(Math.round(avgClosing))} sub={`${balances.length} days`} color={C.accent}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20,marginBottom:20}}>
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:24}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Field label="Opening Balance" required><input type="number" style={iStyle} placeholder="0.00" value={form.opening_balance} onChange={e=>setForm({...form,opening_balance:e.target.value})}/></Field>
            <Field label="Closing Balance" required><input type="number" style={iStyle} placeholder="0.00" value={form.closing_balance} onChange={e=>setForm({...form,closing_balance:e.target.value})}/></Field>
          </div>
          {form.opening_balance&&form.closing_balance&&(()=>{const diff=Number(form.closing_balance)-Number(form.opening_balance);return(
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",justifyContent:"space-between"}}>
              <span style={{fontSize:13,fontWeight:600,color:C.muted}}>Daily movement</span>
              <span style={{fontFamily:"'DM Mono',monospace",fontWeight:800,color:diff>=0?C.green:C.red}}>{diff>=0?"+":""}{fmtFull(diff)}</span>
            </div>
          );})()}
          <Field label="Notes"><textarea style={{...iStyle,resize:"vertical",minHeight:60}} placeholder="e.g. Market day" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></Field>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:24}}>
          ...
          :<div style={{display:"grid",gap:8,maxHeight:400,overflowY:"auto"}}>
            {balances.slice(0,15).map(b=>{const diff=b.closing_balance-b.opening_balance;return(
              <div key={b.id} style={{background:C.surface,borderRadius:9,padding:"12px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div><div style={{fontSize:12,fontWeight:700,color:C.muted,fontFamily:"'DM Mono',monospace"}}>{String(b.date)}</div>{b.notes&&<div style={{fontSize:11,fontWeight:500,color:C.dim,marginTop:2}}>{b.notes}</div>}</div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:14,fontWeight:800,color:diff>=0?C.green:C.red}}>{diff>=0?"+":""}{fmtFull(diff)}</div>
                    <div style={{fontSize:11,fontWeight:600,color:C.muted,marginTop:2}}>close: {fmtFull(b.closing_balance)}</div>
                    {canDelete&&<button onClick={()=>setConfirmCash(b)} style={{fontSize:10,fontWeight:700,color:C.red,background:"transparent",border:`1px solid ${C.red}33`,borderRadius:4,padding:"2px 6px",cursor:"pointer",marginTop:4,fontFamily:"'IBM Plex Sans',sans-serif"}}>✕ delete</button>}
                  </div>
                </div>
              </div>
            );})}
      ...
      {chartData.length>1&&<ChartCard title="Opening vs Closing — Last 30 Days" height={240}>
